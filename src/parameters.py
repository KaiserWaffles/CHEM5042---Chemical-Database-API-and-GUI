...
def pass_ro3(mw: float, logp: float, hbd: int, hba: int, hatom: int, rotb: int, tpsa: float) -> bool:
    """Rule of 3; fragment-like filter."""
    return (mw <= 300) and (logp <= 3) and (hbd <= 3) and (hba <= 3) and (hatom < 20) and (rotb <= 3) and (tpsa <= 60)


def pass_lead_like(mw: float, logp: float, hbd: int, hba: int, rotb: int, tpsa: float) -> bool:
    """Lead-like filter"""
    return (250 <= mw <= 350) and (logp <= 3) and (hbd <= 3) and (hba <= 6) and (rotb <= 10) and (tpsa <= 120)


def pass_veber(rotb: int, tpsa: float, hbd: int, hba: int) -> bool:
    """Veber oral bioavailability filter."""
    return (rotb <= 10) and (tpsa <= 140) or ((hbd + hba) <= 12)


def pass_bioavailability(ro5_ok: bool, rotb: int, tpsa: float, rings:int) -> bool:
    """ bioavailability filter; combines Ro5 + Veber."""
    return ro5_ok and (rotb <= 10) and (tpsa <= 200) and (rings <= 5)
...
def compute_properties(mol: Chem.Mol) -> Parameters:
    ...
    ro5_ok = pass_ro5(v)
    ro3_ok = pass_ro3(mw, logp, hbd, hba, hatom, rotb, tpsa)
    lead_ok = pass_lead_like(mw, logp, hbd, hba, rotb, tpsa)
    veber_ok = pass_veber(rotb, tpsa, hbd, hba)
    bio_ok = pass_bioavailability(ro5_ok, rotb, tpsa, rings)
...
FILTER_DESCRIPTIONS = {
    "Lipinski Ro5": (
        ...
        "  Hydrogen Bond Acceptors <= 10 (sum of N and O)"
    ),
    "Rule of 3 (Fragment)": (
    ...
    "Lead-likeness": (
        ...
        "ALL must be satisfied:\n"
        "  250 <= Molecular Weight <= 350 Da\n"
        "  LogP  <= 3\n"
        "  Hydrogen Bond Donors   <= 3\n"
        "  Hydrogen Bond Acceptors   <= 6\n"
        "  Rotatable bonds <= 7\n"
        "  Polar Surface Area  <= 120 A^2"
    ),
    "Veber Rules": (
